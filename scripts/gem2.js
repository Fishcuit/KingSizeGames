...
function handleCellClick(cell) {
  ...
  if (selectedCells === 3) {
    if (checkForPerfectMatch()) {
      ...
      // Add a different symbol to the game board
      addNewSymbol();
      // If a perfect match is found in the first 3 selections, prevent further selections
      return;
    }
  }

  if (selectedCells >= 5 || (selectedCells > 3 && checkForMatch())) {
    console.log("Match");
    return;
  }
  ...
  symbol.style.backgroundColor = "rgba(113, 42, 228, 1)";
  selectedCells++;
  ...
  if (selectedCells >= 3) {
    ...
    if (checkForMatch()) {
      ...
      // Reveal all symbols before showing the end game modal
      const cells = document.querySelectorAll(".grid-item");
      cells.forEach((cell) => {
        const cover = cell.querySelector(".cover");
        const symbol = cell.querySelector(".symbol");
...
function checkForMatch() {
  ...
  if (matchingSymbol) {
    if (tryCount === 3) score += 3;
    else if (tryCount === 4) score += 2;
    else if (tryCount === 5) score += 1;
    selectedSymbols.forEach((symbol) => {
...
function addNewSymbol() {
  // Add a new symbol to the game board at a random location
  const newSymbol = "🚀"; // the new symbol
