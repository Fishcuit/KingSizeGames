...
function populateGrid(gridArray) {
  ...
  let symbolArray = new Array(12)
    .fill(symbols[0])
    .concat(new Array(13).fill(symbols[1]));
...
function handleCellClick(event) {
  ...
  let revealedSymbols = Array.from(grid.children)
      .filter(cell => cell.textContent !== "" && cell.style.backgroundImage === "none");

  let matchFound = false; // Add a flag variable to track if a match has been found

  // Find all combinations of three revealed symbols
  for (let i = 0; i < revealedSymbols.length - 2; i++) {
      for (let j = i + 1; j < revealedSymbols.length - 1; j++) {
          for (let k = j + 1; k < revealedSymbols.length; k++) {
              // Check if the combination has three matching symbols
              if (
                  revealedSymbols[i].dataset.symbol === revealedSymbols[j].dataset.symbol &&
                  revealedSymbols[j].dataset.symbol === revealedSymbols[k].dataset.symbol
              ) {
                  // Replace matched symbols with the next symbol in the list
                  [revealedSymbols[i], revealedSymbols[j], revealedSymbols[k]].forEach(cell => {
                  ...
                  Array.from(grid.children).forEach(cell => {
                      cell.textContent = "";
                      cell.style.backgroundImage = 'url("img/chest.png")';
                  });
          ...
          if (matchFound) break; // Break out of the middle loop if a match was found
      }
      if (matchFound) break; // Break out of the outer loop if a match was found
  }
  ...
  if (attemptCounter === 0) {
    ...
    document.getElementById("bonusGame").style.display = "none";
  }
}
